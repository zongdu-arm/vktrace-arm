...
vkReplay::vkReplay(vkreplayer_settings *pReplaySettings, vktrace_trace_file_header *pFileHeader,
    ...
    m_minTraceBufferDeviceAddress = UINT64_MAX;
    m_maxTraceBufferDeviceAddress = 0;
    curSwapchainHandle = VK_NULL_HANDLE;
    curSwapchainImage = VK_NULL_HANDLE;
}
...
vkReplay::~vkReplay() {
    ...
    for (auto& e : replayMicromapToCopyAddress) {
        free(e.second);
    }
...
VkResult vkReplay:: manually_replay_vkCreateMicromapEXT(packet_vkCreateMicromapEXT* pPacket) {
    VkResult replayResult = VK_ERROR_VALIDATION_FAILED_EXT;

    VkMicromapEXT local_pMicromap;
    VkDevice remappeddevice = m_objMapper.remap_devices(pPacket->device);
    if (pPacket->device != VK_NULL_HANDLE && remappeddevice == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CreateMicromapEXT() due to invalid remapped VkDevice.");
        return replayResult;
    }
    ...
    replayResult = m_vkDeviceFuncs.CreateMicromapEXT(remappeddevice, pPacket->pCreateInfo, pPacket->pAllocator, &local_pMicromap);
...
void vkReplay:: manually_replay_vkDestroyMicromapEXT(packet_vkDestroyMicromapEXT* pPacket) {
    VkDevice remappeddevice = m_objMapper.remap_devices(pPacket->device);
    if (pPacket->device != VK_NULL_HANDLE && remappeddevice == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in DestroyMicromapEXT() due to invalid remapped VkDevice.");
        return;
    }
    VkMicromapEXT remappedmicromap = m_objMapper.remap_micromapexts(pPacket->micromap);
    if (pPacket->micromap != VK_NULL_HANDLE && remappedmicromap == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in DestroyMicromapEXT() due to invalid remapped VkMicromapEXT.");
        return;
    }
    // No need to remap pAllocator
...
VkResult vkReplay:: manually_replay_vkBuildMicromapsEXT(packet_vkBuildMicromapsEXT* pPacket) {
    ...
    VkDevice remappeddevice = m_objMapper.remap_devices(pPacket->device);
    if (pPacket->device != VK_NULL_HANDLE && remappeddevice == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in BuildMicromapsEXT() due to invalid remapped VkDevice.");
    ...
    if (pPacket->deferredOperation != VK_NULL_HANDLE && remappeddeferredOperation == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in BuildMicromapsEXT() due to invalid remapped VkDeferredOperationKHR.");
        return VK_ERROR_VALIDATION_FAILED_EXT;
    }
    ...
    for (int i = 0; i < pPacket->infoCount; i++) {
        ...
        if (pPacket->pInfos[i].dstMicromap == VK_NULL_HANDLE) {
            vktrace_LogError("Error detected in BuildMicromapsEXT() due to invalid remapped VkMicromapEXT.");
            return replayResult;
        }
    }
    replayResult = m_vkDeviceFuncs.BuildMicromapsEXT(remappeddevice, remappeddeferredOperation, pPacket->infoCount, pPacket->pInfos);
...
VkResult vkReplay:: manually_replay_vkCopyMicromapEXT(packet_vkCopyMicromapEXT* pPacket) {
    VkResult replayResult = VK_ERROR_VALIDATION_FAILED_EXT;

    VkDevice remappeddevice = m_objMapper.remap_devices(pPacket->device);
    if (pPacket->device != VK_NULL_HANDLE && remappeddevice == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CopyMicromapEXT() due to invalid remapped VkDevice.");
        return replayResult;
    }
    VkDeferredOperationKHR remappeddeferredOperation = m_objMapper.remap_deferredoperationkhrs(pPacket->deferredOperation);
    if (pPacket->deferredOperation != VK_NULL_HANDLE && remappeddeferredOperation == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CopyMicromapEXT() due to invalid remapped VkDeferredOperationKHR.");
        return replayResult;
    }
    remappeddeferredOperation = VK_NULL_HANDLE;

    VkMicromapEXT remappedmicromap_src = m_objMapper.remap_micromapexts(pPacket->pInfo->src);
    if (pPacket->pInfo->src != VK_NULL_HANDLE && remappedmicromap_src == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CopyMicromapEXT() due to invalid remapped source micromap.");
        return replayResult;
    }
    VkMicromapEXT remappedmicromap_dst = m_objMapper.remap_micromapexts(pPacket->pInfo->dst);
    if (pPacket->pInfo->dst != VK_NULL_HANDLE && remappedmicromap_dst == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CopyMicromapEXT() due to invalid remapped target micromap.");
        return replayResult;
    }

    const_cast<VkCopyMicromapInfoEXT*>(pPacket->pInfo)->src = remappedmicromap_src;
    const_cast<VkCopyMicromapInfoEXT*>(pPacket->pInfo)->dst = remappedmicromap_dst;

    replayResult = m_vkDeviceFuncs.CopyMicromapEXT(remappeddevice, remappeddeferredOperation, pPacket->pInfo);

    return replayResult;
}

void vkReplay:: manually_replay_vkCmdCopyMicromapEXT(packet_vkCmdCopyMicromapEXT* pPacket) {
    VkCommandBuffer remappedcommandBuffer = m_objMapper.remap_commandbuffers(pPacket->commandBuffer);
    if (pPacket->commandBuffer != VK_NULL_HANDLE && remappedcommandBuffer == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CmdCopyMicromapEXT() due to invalid remapped VkCommandBuffer.");
        return;
    }

    VkMicromapEXT remappedmicromap_src = m_objMapper.remap_micromapexts(pPacket->pInfo->src);
    if (pPacket->pInfo->src != VK_NULL_HANDLE && remappedmicromap_src == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CmdCopyMicromapEXT() due to invalid remapped source micromap.");
        return;
    }
    VkMicromapEXT remappedmicromap_dst = m_objMapper.remap_micromapexts(pPacket->pInfo->dst);
    if (pPacket->pInfo->dst != VK_NULL_HANDLE && remappedmicromap_dst == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CmdCopyMicromapEXT() due to invalid remapped target micromap.");
        return;
    }
    ...
    m_vkDeviceFuncs.CmdCopyMicromapEXT(remappedcommandBuffer, pPacket->pInfo);
}

VkResult vkReplay:: manually_replay_vkCopyMicromapToMemoryEXT(packet_vkCopyMicromapToMemoryEXT* pPacket) {
    VkResult replayResult = VK_ERROR_VALIDATION_FAILED_EXT;

    VkDevice remappeddevice = m_objMapper.remap_devices(pPacket->device);
    if (pPacket->device != VK_NULL_HANDLE && remappeddevice == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CopyMicromapToMemoryEXT() due to invalid remapped VkDevice.");
        return replayResult;
    }
    VkDeferredOperationKHR remappeddeferredOperation = m_objMapper.remap_deferredoperationkhrs(pPacket->deferredOperation);
    if (pPacket->deferredOperation != VK_NULL_HANDLE && remappeddeferredOperation == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CopyMicromapToMemoryEXT() due to invalid remapped VkDeferredOperationKHR.");
        return replayResult;
    }
    remappeddeferredOperation = VK_NULL_HANDLE;
    ...
    VkMicromapEXT remappedmicromap_src = m_objMapper.remap_micromapexts(pPacket->pInfo->src);
    if (pPacket->pInfo->src != VK_NULL_HANDLE && remappedmicromap_src == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CopyMicromapToMemoryEXT() due to invalid remapped source micromap.");
        return replayResult;
    }

    const_cast<VkCopyMicromapToMemoryInfoEXT*>(pPacket->pInfo)->src = remappedmicromap_src;

    assert(pPacket->pInfo->dst.hostAddress != nullptr);

    if (isMapMemoryAddress(pPacket->pInfo->dst.hostAddress)) {
        const_cast<VkCopyMicromapToMemoryInfoEXT*>(pPacket->pInfo)->dst.hostAddress = fromTraceAddr2ReplayAddr(remappeddevice, pPacket->pInfo->dst.hostAddress);
    } else {
        auto it = replayMicromapToMicromapBuildSizes.find(pPacket->pInfo->src);
        if (it != replayMicromapToMicromapBuildSizes.end()) {
            void* dst = malloc(it->second.micromapSize);
            const_cast<VkCopyMicromapToMemoryInfoEXT*>(pPacket->pInfo)->dst.hostAddress = dst;
            replayMicromapToCopyAddress[pPacket->pInfo->src] = dst;
        } else {
            vktrace_LogError("Error detected in CopyMicromapToMemoryEXT(): The src micromap is not exist.");
            return replayResult;
        }
    }

    replayResult = m_vkDeviceFuncs.CopyMicromapToMemoryEXT(remappeddevice, remappeddeferredOperation, pPacket->pInfo);

    return replayResult;
}

void vkReplay:: manually_replay_vkCmdCopyMicromapToMemoryEXT(packet_vkCmdCopyMicromapToMemoryEXT* pPacket) {
    VkCommandBuffer remappedcommandBuffer = m_objMapper.remap_commandbuffers(pPacket->commandBuffer);
    if (pPacket->commandBuffer != VK_NULL_HANDLE && remappedcommandBuffer == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CmdCopyMicromapToMemoryEXT() due to invalid remapped VkCommandBuffer.");
        return;
    }

    VkMicromapEXT remappedmicromap_src = m_objMapper.remap_micromapexts(pPacket->pInfo->src);
    if (pPacket->pInfo->src != VK_NULL_HANDLE && remappedmicromap_src == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CmdCopyMicromapToMemoryEXT() due to invalid remapped source micromap.");
        return;
    }
    const_cast<VkCopyMicromapToMemoryInfoEXT*>(pPacket->pInfo)->src = remappedmicromap_src;

    // if the target address is the address recorded before
    auto it = traceDeviceAddrToReplayDeviceAddr4Buf.find(pPacket->pInfo->dst.deviceAddress);
    if (it != traceDeviceAddrToReplayDeviceAddr4Buf.end()) {
        const_cast<VkCopyMicromapToMemoryInfoEXT*>(pPacket->pInfo)->dst.deviceAddress = it->second.replayDeviceAddr;
    }
    else {
        // TODO, find an alternative to remap the target device address
        vktrace_LogError("Error detected in CmdCopyMicromapToMemoryEXT() due to invalid remapped target device address.");
        return;
    }

    m_vkDeviceFuncs.CmdCopyMicromapToMemoryEXT(remappedcommandBuffer, pPacket->pInfo);
}

VkResult vkReplay:: manually_replay_vkCopyMemoryToMicromapEXT(packet_vkCopyMemoryToMicromapEXT* pPacket) {
    VkResult replayResult = VK_ERROR_VALIDATION_FAILED_EXT;

    VkDevice remappeddevice = m_objMapper.remap_devices(pPacket->device);
    if (pPacket->device != VK_NULL_HANDLE && remappeddevice == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CopyMemoryToMicromapEXT() due to invalid remapped VkDevice.");
        return replayResult;
    }
    VkDeferredOperationKHR remappeddeferredOperation = m_objMapper.remap_deferredoperationkhrs(pPacket->deferredOperation);
    if (pPacket->deferredOperation != VK_NULL_HANDLE && remappeddeferredOperation == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CopyMemoryToMicromapEXT() due to invalid remapped VkDeferredOperationKHR.");
        return replayResult;
    }
    ...
    VkMicromapEXT remappedmicromap_dst = m_objMapper.remap_micromapexts(pPacket->pInfo->dst);
    if (pPacket->pInfo->dst != VK_NULL_HANDLE && remappedmicromap_dst == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CopyMemoryToMicromapEXT() due to invalid remapped target micromap.");
        return replayResult;
    }
    const_cast<VkCopyMemoryToMicromapInfoEXT*>(pPacket->pInfo)->dst = remappedmicromap_dst;

    assert(pPacket->pInfo->src.hostAddress != nullptr);

    if (isMapMemoryAddress(pPacket->pInfo->src.hostAddress)) {
        const_cast<VkCopyMemoryToMicromapInfoEXT*>(pPacket->pInfo)->src.hostAddress = fromTraceAddr2ReplayAddr(remappeddevice, pPacket->pInfo->src.hostAddress);
    } else {
        auto it = replayMicromapToCopyAddress.find(pPacket->pInfo->dst);
        if (it != replayMicromapToCopyAddress.end()) {
            const_cast<VkCopyMemoryToMicromapInfoEXT*>(pPacket->pInfo)->src.hostAddress = it->second;
        } else {
            vktrace_LogError("vkTrace doesn't support vkCopyMemoryToMicromapEXT.");
        }
    }

    replayResult = m_vkDeviceFuncs.CopyMemoryToMicromapEXT(remappeddevice, remappeddeferredOperation, pPacket->pInfo);

    return replayResult;
}

void vkReplay:: manually_replay_vkCmdCopyMemoryToMicromapEXT(packet_vkCmdCopyMemoryToMicromapEXT* pPacket) {
    VkCommandBuffer remappedcommandBuffer = m_objMapper.remap_commandbuffers(pPacket->commandBuffer);
    if (pPacket->commandBuffer != VK_NULL_HANDLE && remappedcommandBuffer == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CmdCopyMemoryToMicromapEXT() due to invalid remapped VkCommandBuffer.");
        return;
    }

    VkMicromapEXT remappedmicromap_dst = m_objMapper.remap_micromapexts(pPacket->pInfo->dst);
    if (pPacket->pInfo->dst != VK_NULL_HANDLE && remappedmicromap_dst == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CmdCopyMemoryToMicromapEXT() due to invalid remapped target micromap.");
        return;
    }
    const_cast<VkCopyMemoryToMicromapInfoEXT*>(pPacket->pInfo)->dst = remappedmicromap_dst;

    // if the source address is the address recorded before
    auto it = traceDeviceAddrToReplayDeviceAddr4Buf.find(pPacket->pInfo->src.deviceAddress);
    if (it != traceDeviceAddrToReplayDeviceAddr4Buf.end()) {
        const_cast<VkCopyMemoryToMicromapInfoEXT*>(pPacket->pInfo)->src.deviceAddress = it->second.replayDeviceAddr;
    }
    else {
        // TODO, find an alternative to remap the source device address
        vktrace_LogError("Error detected in CmdCopyMemoryToMicromapEXT() due to invalid remapped source device address.");
        return;
    }

    m_vkDeviceFuncs.CmdCopyMemoryToMicromapEXT(remappedcommandBuffer, pPacket->pInfo);
}

VkResult vkReplay:: manually_replay_vkWriteMicromapsPropertiesEXT(packet_vkWriteMicromapsPropertiesEXT* pPacket) {
    VkResult replayResult = VK_ERROR_VALIDATION_FAILED_EXT;

    VkDevice remappeddevice = m_objMapper.remap_devices(pPacket->device);
    if (pPacket->device != VK_NULL_HANDLE && remappeddevice == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in WriteMicromapsPropertiesEXT() due to invalid remapped VkDevice.");
        return replayResult;
    }
    // No need to remap micromapCount
    VkMicromapEXT *remappedpMicromaps = new VkMicromapEXT[pPacket->micromapCount];
    for (uint32_t i = 0; i < pPacket->micromapCount; i++) {
        remappedpMicromaps[i] = m_objMapper.remap_micromapexts(pPacket->pMicromaps[i]);
        if (pPacket->pMicromaps[i] != VK_NULL_HANDLE && remappedpMicromaps[i] == VK_NULL_HANDLE) {
            vktrace_LogError("Error detected in WriteMicromapsPropertiesEXT() due to invalid remapped VkMicromapEXT.");
            delete []remappedpMicromaps;
            return replayResult;
        }
    }
    ...
    if (isMapMemoryAddress(pPacket->pData)) {
        pPacket->pData = fromTraceAddr2ReplayAddr(remappeddevice, pPacket->pData);
    } else {
        // TODO
    }
    // No need to remap stride
    replayResult = m_vkDeviceFuncs.WriteMicromapsPropertiesEXT(remappeddevice, pPacket->micromapCount, remappedpMicromaps, pPacket->queryType, pPacket->dataSize, pPacket->pData, pPacket->stride);
...
void vkReplay:: manually_replay_vkCmdWriteMicromapsPropertiesEXT(packet_vkCmdWriteMicromapsPropertiesEXT* pPacket) {
    VkCommandBuffer remappedcommandBuffer = m_objMapper.remap_commandbuffers(pPacket->commandBuffer);
    if (pPacket->commandBuffer != VK_NULL_HANDLE && remappedcommandBuffer == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CmdWriteMicromapsPropertiesEXT() due to invalid remapped VkCommandBuffer.");
        return;
    }
    // No need to remap micromapCount
    VkMicromapEXT *remappedpMicromaps = new VkMicromapEXT[pPacket->micromapCount];
    for (uint32_t i = 0; i < pPacket->micromapCount; i++) {
        remappedpMicromaps[i] = m_objMapper.remap_micromapexts(pPacket->pMicromaps[i]);
        if (pPacket->pMicromaps[i] != VK_NULL_HANDLE && remappedpMicromaps[i] == VK_NULL_HANDLE) {
            vktrace_LogError("Error detected in CmdWriteMicromapsPropertiesEXT() due to invalid remapped VkMicromapEXT.");
            delete []remappedpMicromaps;
            return;
        }
    }
    ...
    if (pPacket->queryPool != VK_NULL_HANDLE && remappedqueryPool == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CmdWriteMicromapsPropertiesEXT() due to invalid remapped VkQueryPool.");
        delete []remappedpMicromaps;
        return;
    }
    // No need to remap firstQuery
    m_vkDeviceFuncs.CmdWriteMicromapsPropertiesEXT(remappedcommandBuffer, pPacket->micromapCount, remappedpMicromaps, pPacket->queryType, remappedqueryPool, pPacket->firstQuery);

    if (pPacket->queryType == VK_QUERY_TYPE_MICROMAP_COMPACTED_SIZE_EXT) {
        for (int i = 0; i < pPacket->micromapCount; ++i)
            replayQueryPoolMicromapCompactSize[remappedqueryPool].insert(pPacket->firstQuery + i);
    }
    delete []remappedpMicromaps;
}

void vkReplay:: manually_replay_vkGetDeviceMicromapCompatibilityEXT(packet_vkGetDeviceMicromapCompatibilityEXT* pPacket) {
    VkDevice remappeddevice = m_objMapper.remap_devices(pPacket->device);
    if (pPacket->device != VK_NULL_HANDLE && remappeddevice == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in GetDeviceMicromapCompatibilityEXT() due to invalid remapped VkDevice.");
        return;
    }
    // No need to remap pVersionInfo
    // No need to remap pCompatibility
    m_vkDeviceFuncs.GetDeviceMicromapCompatibilityEXT(remappeddevice, pPacket->pVersionInfo, pPacket->pCompatibility);
}

void vkReplay:: manually_replay_vkGetMicromapBuildSizesEXT(packet_vkGetMicromapBuildSizesEXT* pPacket) {
    VkDevice remappeddevice = m_objMapper.remap_devices(pPacket->device);
    if (pPacket->device != VK_NULL_HANDLE && remappeddevice == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in GetMicromapBuildSizesEXT() due to invalid remapped VkDevice.");
        return;
    }

    // No need to remap buildType
    // No need to remap pBuildInfo
...
void vkReplay::createCommandPoolAndCommandBuffer(VkResult& replayResult) {
    ...
    if(replayResult != VK_SUCCESS) {
        vktrace_LogError("Failed to create command pool when converting the Android frame boundary");
    }
    ...
    if (replaySettings.useEXTFrameBoundary) {
        VkCommandBufferBeginInfo commandBufferBeginInfo;
        commandBufferBeginInfo.sType            = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
...
VkResult vkReplay::manually_replay_vkFrameBoundaryANDROID(packet_vkFrameBoundaryANDROID* pPacket) {
    ...
    // use VK_EXT_frame_boundary, convert VK_ANDROID_frame_boundary to VK_EXT_frame_boundary
    if (replaySettings.useEXTFrameBoundary) {
        ...
        frameBoundaryExt.frameID     = m_frameNumber++;
        frameBoundaryExt.imageCount  = (remappedImage == VK_NULL_HANDLE ? 0 : 1);
        frameBoundaryExt.pImages     = (remappedImage == VK_NULL_HANDLE ? nullptr : &remappedImage);
        ...
        // vkFrameBoundaryANDROID doesn't signal or wait on semaphore, so keep semaphore state unchanged here
        ...
        submitInfo.waitSemaphoreCount   = 1;
        submitInfo.pWaitSemaphores      = &remappedSemaphore;
        ...
        replayResult = m_vkDeviceFuncs.QueueSubmit(g_queue, 1, &submitInfo, VK_NULL_HANDLE);
...
VkResult vkReplay::manually_replay_vkGetSemaphoreFdKHR(packet_vkGetSemaphoreFdKHR* pPacket) {
    if (replaySettings.useEXTFrameBoundary) {
        // skip this call if the semaphore is waited in the previous inserted queuesubmit.
        if (frameboundarySemaphores.find(pPacket->pGetFdInfo->semaphore) != frameboundarySemaphores.end())
            return pPacket->result;
    }
    VkResult replayResult = VK_ERROR_VALIDATION_FAILED_EXT;
    VkDevice remappeddevice = m_objMapper.remap_devices(pPacket->device);
    if (pPacket->device != VK_NULL_HANDLE && remappeddevice == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in GetSemaphoreFdKHR() due to invalid remapped VkDevice.");
        return replayResult;
    }
    // No need to remap pGetFdInfo
    // No need to remap pFd
    VkSemaphore semaphore = m_objMapper.remap_semaphores(pPacket->pGetFdInfo->semaphore);
    if (pPacket->pGetFdInfo->semaphore != VK_NULL_HANDLE && semaphore == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in GetSemaphoreFdKHR() due to invalid remapped VkSemaphore.");
        return replayResult;
    }
    const_cast<VkSemaphoreGetFdInfoKHR*>(pPacket->pGetFdInfo)->semaphore = semaphore;
    replayResult = m_vkDeviceFuncs.GetSemaphoreFdKHR(remappeddevice, pPacket->pGetFdInfo, pPacket->pFd);
    return replayResult;
}
...
VkResult vkReplay::manually_replay_vkCreateInstance(packet_vkCreateInstance *pPacket) {
    ...
    bool foundReplayExtDEvProp2 = false;
    for (uint32_t i = 0; i < extensionCount; i++) {
        // Check extension is replayable
        if (strcmp(extensions[i].extensionName, VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME) == 0) {
            foundReplayExtDEvProp2 = true;
            break;
        }
    }

    if (!foundEnabledExtDEvProp2 && foundReplayExtDEvProp2) {
...
void vkReplay::checkDeviceExtendFeatures(const VkBaseOutStructure *pNext, VkPhysicalDevice physicalDevice) {
    ...
    while(pNext != nullptr) {
        switch(pNext->sType) {
            ...
            CHECKDEVICEFEATURES(VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_OPACITY_MICROMAP_FEATURES_EXT, VkPhysicalDeviceOpacityMicromapFeaturesEXT, 3)
            CHECKDEVICEFEATURES(VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SYNCHRONIZATION_2_FEATURES, VkPhysicalDeviceSynchronization2Features, 1)
            default:
...
void vkReplay::forceDisableCaptureReplayFeature() {
    ...
    if (g_hasAsApi && (m_pFileHeader->bit_flags & VKTRACE_RQ_POSTPROCESSED_BIT)) {
        bForceDisable = true;
    }
    if (bForceDisable) {
...
VkResult vkReplay::manually_replay_vkCreateDevice(packet_vkCreateDevice *pPacket) {
    ...
    // VK_KHR_acceleration_structure function maybe need VK_KHR_buffer_device_address extension
    bool foundEnabledAsExt = false;
    for (uint32_t i = 0; i < pCreateInfo->enabledExtensionCount; i++) {
        // Check extension is replayable
        if (strcmp(pCreateInfo->ppEnabledExtensionNames[i], VK_KHR_ACCELERATION_STRUCTURE_EXTENSION_NAME) == 0) {
            foundEnabledAsExt = true;
            break;
        }
    }

    bool foundEnabledBufAddrExt = false;
    for (uint32_t i = 0; i < pCreateInfo->enabledExtensionCount; i++) {
        // Check extension is replayable
        if (strcmp(pCreateInfo->ppEnabledExtensionNames[i], VK_KHR_BUFFER_DEVICE_ADDRESS_EXTENSION_NAME) == 0) {
            foundEnabledBufAddrExt = true;
            break;
        }
    }

    bool foundReplayAsExt = false;
    for (uint32_t i = 0; i < extensionCount; i++) {
        // Check extension is replayable
        if (strcmp(extensions[i].extensionName, VK_KHR_BUFFER_DEVICE_ADDRESS_EXTENSION_NAME) == 0) {
            foundReplayAsExt = true;
            break;
        }
    }

    if (!foundEnabledBufAddrExt && foundEnabledAsExt && foundReplayAsExt) {
        extensionNames.push_back(VK_KHR_BUFFER_DEVICE_ADDRESS_EXTENSION_NAME);
    }
    if (g_pReplaySettings->insertDeviceExtension != NULL) {
        extensionNames.push_back(g_pReplaySettings->insertDeviceExtension);
    }
    ...
    if (g_pReplaySettings->forceVariableRateShading != nullptr) {
        ...
        if (VK_SUCCESS !=
            ...
            std::vector<VkPhysicalDeviceFragmentShadingRateKHR> fragmentShadingRates(fragmentShadingRateCount);
            for (auto& fragmentShadingRate : fragmentShadingRates) {
            ...
            vktrace_LogAlways("fragmentShadingRateCount = %d", fragmentShadingRateCount);
            bool foundFragmentSize = false;
            for (uint32_t i = 0; i < fragmentShadingRateCount; i++) {
                // SampleCount is ~0 if fragment size is {1, 1}
                vktrace_LogAlways("Supported fragmentShadingRate: sampleCount = %u, width = %u, height = %u.",
                ...
                if (m_fragmentSize.width == fragmentShadingRates[i].fragmentSize.width &&
                    ...
                    vktrace_LogWarning("Input fragment size found.");
                }
            }
    ...
    if (g_pReplaySettings->enableVirtualSwapchain && g_pReplaySettings->forceSyncImgIdx) {
        vktrace_LogWarning("fsii: Enable enableVirtualSwapchain option, then forceSyncImgIdx will be disabled.");
        g_pReplaySettings->forceSyncImgIdx = false;
    }
...
VkResult vkReplay::manually_replay_vkEnumeratePhysicalDeviceGroups(packet_vkEnumeratePhysicalDeviceGroups *pPacket) {
    ...
    VkInstance remappedInstance = m_objMapper.remap_instances(pPacket->instance);
    if (remappedInstance == VK_NULL_HANDLE) {
        vktrace_LogError("Skipping vkEnumeratePhysicalDeviceGroups() due to invalid remapped VkInstance.");
        return VK_ERROR_VALIDATION_FAILED_EXT;
    }
...
VkResult vkReplay::manually_replay_vkQueueSubmit2(packet_vkQueueSubmit2 *pPacket) {
    VkResult replayResult = VK_ERROR_VALIDATION_FAILED_EXT;
    VkQueue remappedQueue = m_objMapper.remap_queues(pPacket->queue);
    if (remappedQueue == VK_NULL_HANDLE) {
        vktrace_LogError("Skipping vkQueueSubmit2() due to invalid remapped VkQueue.");
        return VK_ERROR_VALIDATION_FAILED_EXT;
    }

    VkFence remappedFence = m_objMapper.remap_fences(pPacket->fence);
    if (pPacket->fence != VK_NULL_HANDLE && remappedFence == VK_NULL_HANDLE) {
        vktrace_LogError("Skipping vkQueueSubmit2() due to invalid remapped VkPhysicalDevice.");
        return VK_ERROR_VALIDATION_FAILED_EXT;
    }

    VkSubmitInfo2 *remappedSubmits = (VkSubmitInfo2 *)pPacket->pSubmits;

    for (uint32_t submit_idx = 0; submit_idx < pPacket->submitCount; submit_idx++) {
        const VkSubmitInfo2 *submit = &pPacket->pSubmits[submit_idx];
        VkSubmitInfo2 *remappedSubmit = &remappedSubmits[submit_idx];
        // Remap Semaphores & CommandBuffers for this submit
        uint32_t i = 0;
        if (submit->pCommandBufferInfos != NULL) {
            VkCommandBufferSubmitInfo *pRemappedBuffers = (VkCommandBufferSubmitInfo *)remappedSubmit->pCommandBufferInfos;
            for (i = 0; i < submit->commandBufferInfoCount; i++) {
                pRemappedBuffers[i].commandBuffer = m_objMapper.remap_commandbuffers(submit->pCommandBufferInfos[i].commandBuffer);
                if (pRemappedBuffers[i].commandBuffer == VK_NULL_HANDLE) {
                    vktrace_LogError("Skipping vkQueueSubmit2() due to invalid remapped VkCommandBuffer.");
                    return replayResult;
                }
            }
        }
        if (submit->pWaitSemaphoreInfos != NULL) {
            VkSemaphoreSubmitInfo *pRemappedWaitSems = (VkSemaphoreSubmitInfo *)remappedSubmit->pWaitSemaphoreInfos;
            for (i = 0; i < submit->waitSemaphoreInfoCount; i++) {
                pRemappedWaitSems[i].semaphore = m_objMapper.remap_semaphores(submit->pWaitSemaphoreInfos[i].semaphore);
                if (g_pReplaySettings->forceSyncImgIdx) {
                    if (acquireSemaphoreToFSIISemaphore.find(pRemappedWaitSems[i].semaphore) != acquireSemaphoreToFSIISemaphore.end()) {
                        pRemappedWaitSems[i].semaphore = acquireSemaphoreToFSIISemaphore[pRemappedWaitSems[i].semaphore];
                    }
                }
                if (pRemappedWaitSems[i].semaphore == VK_NULL_HANDLE) {
                    vktrace_LogError("Skipping vkQueueSubmit2() due to invalid remapped wait VkSemaphore.");
                    return replayResult;
                }
            }
        }
        if (submit->pSignalSemaphoreInfos != NULL) {
            VkSemaphoreSubmitInfo *pRemappedSignalSems = (VkSemaphoreSubmitInfo *)remappedSubmit->pSignalSemaphoreInfos;
            for (i = 0; i < submit->signalSemaphoreInfoCount; i++) {
                pRemappedSignalSems[i].semaphore = m_objMapper.remap_semaphores(submit->pSignalSemaphoreInfos[i].semaphore);
                if (pRemappedSignalSems[i].semaphore == VK_NULL_HANDLE) {
                    vktrace_LogError("Skipping vkQueueSubmit2() due to invalid remapped signal VkSemaphore.");
                    return replayResult;
                }
            }
        }
    }
    replayResult = m_vkDeviceFuncs.QueueSubmit2(remappedQueue, pPacket->submitCount, remappedSubmits, remappedFence);
#if VK_ANDROID_frame_boundary
    g_queue = remappedQueue;
#endif
    return replayResult;
...
VkResult vkReplay::manually_replay_vkCreateGraphicsPipelines(packet_vkCreateGraphicsPipelines *pPacket) {
    ...
    for (i = 0; i < pPacket->createInfoCount; i++) {
        ...
        if (pCIs[i].layout == VK_NULL_HANDLE) {
            vktrace_LogWarning("Skipping vkCreateGraphicsPipelines() due to invalid remapped VkPipelineLayout.");
        }
        ...
        if (pCIs[i].pViewportState != nullptr) {
        ...
        if (pCIs[i].pMultisampleState != nullptr) {
        ...
        VkPipelineLibraryCreateInfoKHR* pPipelineLibraryCreateInfoKHR = (VkPipelineLibraryCreateInfoKHR *)find_ext_struct((vulkan_struct_header *)&pCIs[i], VK_STRUCTURE_TYPE_PIPELINE_LIBRARY_CREATE_INFO_KHR);
        if (pPipelineLibraryCreateInfoKHR != nullptr) {
            for (int i = 0; i < pPipelineLibraryCreateInfoKHR->libraryCount; i++) {
                const_cast<VkPipeline*>(pPipelineLibraryCreateInfoKHR->pLibraries)[i] = m_objMapper.remap_pipelines(pPipelineLibraryCreateInfoKHR->pLibraries[i]);
            }
        }
        ...
        if (g_pReplaySettings->forceVariableRateShading != nullptr) {
            ...
            if (pPFSRStateCI) {
                ...
                memcpy(pPFSRStateCI, &newPFSRStateCI, sizeof(VkPipelineFragmentShadingRateStateCreateInfoKHR));
...
void vkReplay::manually_replay_vkCmdWaitEvents(packet_vkCmdWaitEvents *pPacket) {
    ...
    for (idx = 0; idx < pPacket->imageMemoryBarrierCount; idx++) {
        ...
        if (g_pReplaySettings->enableVirtualSwapchain && g_pReplaySettings->enableVscPerfMode == FALSE
            && pNextImg->newLayout == VK_IMAGE_LAYOUT_PRESENT_SRC_KHR) {
            pNextImg->newLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        }
...
void vkReplay::manually_replay_vkCmdPipelineBarrier(packet_vkCmdPipelineBarrier *pPacket) {
    ...
    for (idx = 0; idx < pPacket->imageMemoryBarrierCount; idx++) {
        ...
        if (g_pReplaySettings->enableVirtualSwapchain && g_pReplaySettings->enableVscPerfMode == FALSE
            && pNextImg->newLayout == VK_IMAGE_LAYOUT_PRESENT_SRC_KHR) {
            curSwapchainImage = pNextImg->image;
            pNextImg->newLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        }
...
void vkReplay::manually_replay_vkCmdPipelineBarrier2KHR(packet_vkCmdPipelineBarrier2KHR *pPacket) {
    ...
    for (idx = 0; idx < pPacket->pDependencyInfo->bufferMemoryBarrierCount; idx++) {
        ...
        pNextBuf->buffer = m_objMapper.remap_buffers(pNextBuf->buffer);
        if (pNextBuf->buffer == VK_NULL_HANDLE && saveBuf != VK_NULL_HANDLE) {
            vktrace_LogError("Skipping vkCmdPipelineBarrier2KHR() due to invalid remapped VkBuffer.");
            return;
        }
        replayDevice = replayBufferToDevice[pNextBuf->buffer];
    ...
    for (idx = 0; idx < pPacket->pDependencyInfo->imageMemoryBarrierCount; idx++) {
        ...
        pNextImg->image = m_objMapper.remap_images(pNextImg->image);
        if (pNextImg->image == VK_NULL_HANDLE && saveImg != VK_NULL_HANDLE) {
            vktrace_LogError("Skipping vkCmdPipelineBarrier2KHR() due to invalid remapped VkImage.");
            return;
        }
        replayDevice = replayImageToDevice[pNextImg->image];
...
void vkReplay::manually_replay_vkCmdPipelineBarrier2(packet_vkCmdPipelineBarrier2 *pPacket) {
    VkCommandBuffer remappedcommandBuffer = m_objMapper.remap_commandbuffers(pPacket->commandBuffer);
    if (pPacket->commandBuffer != VK_NULL_HANDLE && remappedcommandBuffer == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CmdPipelineBarrier2() due to invalid remapped VkCommandBuffer.");
        return ;
    }
    uint32_t idx = 0;
    VkDevice traceDevice;
    ...
    for (idx = 0; idx < pPacket->pDependencyInfo->bufferMemoryBarrierCount; idx++) {
        ...
        pNextBuf->buffer = m_objMapper.remap_buffers(pNextBuf->buffer);
        if (pNextBuf->buffer == VK_NULL_HANDLE && saveBuf != VK_NULL_HANDLE) {
            vktrace_LogError("Skipping vkCmdPipelineBarrier2() due to invalid remapped VkBuffer.");
            return;
        }
        replayDevice = replayBufferToDevice[pNextBuf->buffer];
        getReplayQueueFamilyIdx(traceDevice, replayDevice, (uint32_t *)&pPacket->pDependencyInfo->pBufferMemoryBarriers[idx].srcQueueFamilyIndex);
    ...
    for (idx = 0; idx < pPacket->pDependencyInfo->imageMemoryBarrierCount; idx++) {
        ...
        pNextImg->image = m_objMapper.remap_images(pNextImg->image);
        if (pNextImg->image == VK_NULL_HANDLE && saveImg != VK_NULL_HANDLE) {
            vktrace_LogError("Skipping vkCmdPipelineBarrier2() due to invalid remapped VkImage.");
            return;
        }
        replayDevice = replayImageToDevice[pNextImg->image];
        getReplayQueueFamilyIdx(traceDevice, replayDevice, (uint32_t *)&pPacket->pDependencyInfo->pImageMemoryBarriers[idx].srcQueueFamilyIndex);
...
VkResult vkReplay::manually_replay_vkCreateFramebuffer(packet_vkCreateFramebuffer *pPacket) {
    ...
    if (pAttachments != NULL && !g_pReplaySettings->enableVirtualSwapchain) {
        for (uint32_t i = 0; i < pInfo->attachmentCount; i++) {
            ...
            SwapchainImageState& curSwapchainImgStat = swapchainImageStates[curSwapchainHandle];
            if (curSwapchainImgStat.traceImageViewToImageIndex.find(savedIV) != curSwapchainImgStat.traceImageViewToImageIndex.end()) {
                curSwapchainImgStat.traceFramebufferToImageIndex[*(pPacket->pFramebuffer)] = curSwapchainImgStat.traceImageViewToImageIndex[savedIV];
...
void vkReplay::manually_replay_vkCmdCopyBufferRemap(packet_vkCmdCopyBuffer *pPacket) {
    VkCommandBuffer remappedcommandBuffer = m_objMapper.remap_commandbuffers(pPacket->commandBuffer);
    if (pPacket->commandBuffer != VK_NULL_HANDLE && remappedcommandBuffer == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CmdCopyBufferRemap() due to invalid remapped VkCommandBuffer.");
        return;
    }
    ...
    if (pPacket->srcBuffer != VK_NULL_HANDLE && remappedsrcBuffer == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CmdCopyBufferRemap() due to invalid remapped VkBuffer.");
        return;
    }
    ...
    if (g_hasAsApi) {
        ...
        if (it == replayCommandBufferToReplayDevice.end()) {
            vktrace_LogError("Error detected in CmdCopyBufferRemap(), couldn't be able to find corresponding VkDevice for VkCommandBuffer 0x%llx.", remappedcommandBuffer);
            return;
        }
        device = it->second;
        ...
        if (it2 == replayBufferToReplayDeviceMemory.end()) {
            vktrace_LogError("Error detected in CmdCopyBufferRemap(), couldn't be able to find corresponding VkDeviceMemory for VkBuffer 0x%llx.", remappedsrcBuffer);
            return;
        }
        mem = it2->second;
        ...
        if (it_offset == replayBufferToReplayDeviceMemoryOffset.end()) {
            vktrace_LogError("Error detected in CmdCopyBufferRemap(), couldn't be able to find corresponding offset for VkBuffer 0x%llx.", remappedsrcBuffer);
            return;
        }
...
void vkReplay::manually_replay_vkCmdBeginRenderingKHR(packet_vkCmdBeginRenderingKHR *pPacket) {
    ...
    if (local_renderingInfo.pDepthAttachment != nullptr) {
        // pDepthAttachment
    ...
    if (local_renderingInfo.pStencilAttachment != nullptr) {
        // pStencilAttachment
...
void vkReplay::manually_replay_vkCmdBeginRendering(packet_vkCmdBeginRendering *pPacket) {
    VkCommandBuffer remappedCommandBuffer = m_objMapper.remap_commandbuffers(pPacket->commandBuffer);
    if (remappedCommandBuffer == VK_NULL_HANDLE) {
        vktrace_LogError("Skipping vkCmdBeginRendering() due to invalid remapped VkCommandBuffer.");
        return;
    }
    VkRenderingInfo local_renderingInfo;
    SwapchainImageState& curSwapchainImgStat = swapchainImageStates[curSwapchainHandle];
    memcpy((void *)&local_renderingInfo, (void *)pPacket->pRenderingInfo, sizeof(VkRenderingInfo));
    local_renderingInfo.pColorAttachments = (const VkRenderingAttachmentInfo *)pPacket->pRenderingInfo->pColorAttachments;

    // pColorAttachments
    VkRenderingAttachmentInfo *local_colorAttachments = VKTRACE_NEW_ARRAY(VkRenderingAttachmentInfo, pPacket->pRenderingInfo->colorAttachmentCount);
    uint32_t i;
    for(i = 0; i < pPacket->pRenderingInfo->colorAttachmentCount; i++) {
        VkRenderingAttachmentInfo local_colorAttachmentInfo = local_renderingInfo.pColorAttachments[i];
        if (curSwapchainImgStat.traceImageViewToImageIndex.find(pPacket->pRenderingInfo->pColorAttachments[i].imageView) != curSwapchainImgStat.traceImageViewToImageIndex.end() &&
            curSwapchainImgStat.traceImageIndexToImageViews.find(m_imageIndex) != curSwapchainImgStat.traceImageIndexToImageViews.end() &&
            m_imageIndex != UINT32_MAX && m_imageIndex != m_pktImgIndex && !g_pReplaySettings->enableVirtualSwapchain) {
            local_colorAttachmentInfo.imageView = m_objMapper.remap_imageviews(*curSwapchainImgStat.traceImageIndexToImageViews[m_imageIndex].begin());
        ...
        local_colorAttachments[i] = local_colorAttachmentInfo;
        if (local_colorAttachmentInfo.imageView == VK_NULL_HANDLE) {
            vktrace_LogError("Skipping vkCmdBeginRendering() due to invalid remapped VkImageView(color attachment [%d]).",  i);
            return;
        }
    }
    local_renderingInfo.pColorAttachments = local_colorAttachments;
    ...
    if (local_renderingInfo.pDepthAttachment != nullptr) {
        // pDepthAttachment
        VkRenderingAttachmentInfo local_depthAttachmentInfo = *local_renderingInfo.pDepthAttachment;
        if (curSwapchainImgStat.traceImageViewToImageIndex.find(pPacket->pRenderingInfo->pDepthAttachment->imageView) != curSwapchainImgStat.traceImageViewToImageIndex.end() &&
            curSwapchainImgStat.traceImageIndexToImageViews.find(m_imageIndex) != curSwapchainImgStat.traceImageIndexToImageViews.end() &&
            m_imageIndex != UINT32_MAX && m_imageIndex != m_pktImgIndex) {
            local_depthAttachmentInfo.imageView = m_objMapper.remap_imageviews(*curSwapchainImgStat.traceImageIndexToImageViews[m_imageIndex].begin());
        ...
        local_renderingInfo.pDepthAttachment = &local_depthAttachmentInfo;
        if (local_renderingInfo.pDepthAttachment->imageView == VK_NULL_HANDLE) {
            vktrace_LogError("Skipping vkCmdBeginRendering() due to invalid remapped VkImageView(depth attachment).");
            return;
        }
    }
    ...
    if (local_renderingInfo.pStencilAttachment != nullptr) {
        // pStencilAttachment
        VkRenderingAttachmentInfo local_stencilAttachmentInfo = *local_renderingInfo.pStencilAttachment;
        if (curSwapchainImgStat.traceImageViewToImageIndex.find(pPacket->pRenderingInfo->pStencilAttachment->imageView) != curSwapchainImgStat.traceImageViewToImageIndex.end() &&
            curSwapchainImgStat.traceImageIndexToImageViews.find(m_imageIndex) != curSwapchainImgStat.traceImageIndexToImageViews.end() &&
            m_imageIndex != UINT32_MAX && m_imageIndex != m_pktImgIndex) {
            local_stencilAttachmentInfo.imageView = m_objMapper.remap_imageviews(*curSwapchainImgStat.traceImageIndexToImageViews[m_imageIndex].begin());
    ...
    m_vkDeviceFuncs.CmdBeginRendering(remappedCommandBuffer, &local_renderingInfo);
    VKTRACE_DELETE(local_colorAttachments);
}

VkResult vkReplay::manually_replay_vkImportSemaphoreFdKHR(packet_vkImportSemaphoreFdKHR* pPacket) {
    VkResult replayResult = VK_ERROR_VALIDATION_FAILED_EXT;
    VkDevice remappeddevice = m_objMapper.remap_devices(pPacket->device);
    if (pPacket->device != VK_NULL_HANDLE && remappeddevice == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in ImportSemaphoreFdKHR() due to invalid remapped VkDevice.");
        return replayResult;
    }
    int fd = -1;
    for (auto it = traceSemaphoreHandleToTraceFD.begin(); it != traceSemaphoreHandleToTraceFD.end(); it++) {
        if (it->second == pPacket->pImportSemaphoreFdInfo->fd) {
            VkSemaphore externalSemaphore = m_objMapper.remap_semaphores(it->first);
            VkSemaphoreGetFdInfoKHR info = {VK_STRUCTURE_TYPE_SEMAPHORE_GET_FD_INFO_KHR, nullptr, externalSemaphore, VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_SYNC_FD_BIT};
            m_vkDeviceFuncs.GetSemaphoreFdKHR(remappeddevice, &info, &fd);
            break;
        }
    }
    if (fd == -1) {
        if (pPacket->pImportSemaphoreFdInfo->handleType == VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_SYNC_FD_BIT) {
            vktrace_LogAlways("Can not find the fd, let fd = -1.");
        } else {
            vktrace_LogError("Can not find the fd = %d, vkImportSemaphoreFdKHR is failed.", pPacket->pImportSemaphoreFdInfo->fd);
        }
    }
    const_cast<VkImportSemaphoreFdInfoKHR*>(pPacket->pImportSemaphoreFdInfo)->fd = fd;
    VkSemaphore semaphore = m_objMapper.remap_semaphores(pPacket->pImportSemaphoreFdInfo->semaphore);
    const_cast<VkImportSemaphoreFdInfoKHR*>(pPacket->pImportSemaphoreFdInfo)->semaphore = semaphore;
    replayResult = m_vkDeviceFuncs.ImportSemaphoreFdKHR(remappeddevice, pPacket->pImportSemaphoreFdInfo);
    return replayResult;
}
...
VkResult vkReplay::manually_replay_vkSignalSemaphore(packet_vkSignalSemaphore* pPacket) {
    VkResult replayResult = VK_ERROR_VALIDATION_FAILED_EXT;
    VkDevice remappeddevice = m_objMapper.remap_devices(pPacket->device);
    if (pPacket->device != VK_NULL_HANDLE && remappeddevice == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in SignalSemaphore() due to invalid remapped VkDevice.");
        return VK_ERROR_VALIDATION_FAILED_EXT;
    }
    const_cast<VkSemaphoreSignalInfo*>(pPacket->pSignalInfo)->semaphore = m_objMapper.remap_semaphores(pPacket->pSignalInfo->semaphore);
    replayResult = m_vkDeviceFuncs.SignalSemaphore(remappeddevice, pPacket->pSignalInfo);
    return replayResult;
}

VkResult vkReplay::manually_replay_vkSignalSemaphoreKHR(packet_vkSignalSemaphoreKHR* pPacket) {
    VkResult replayResult = VK_ERROR_VALIDATION_FAILED_EXT;
    VkDevice remappeddevice = m_objMapper.remap_devices(pPacket->device);
    if (pPacket->device != VK_NULL_HANDLE && remappeddevice == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in SignalSemaphoreKHR() due to invalid remapped VkDevice.");
        return VK_ERROR_VALIDATION_FAILED_EXT;
    }
    const_cast<VkSemaphoreSignalInfo*>(pPacket->pSignalInfo)->semaphore = m_objMapper.remap_semaphores(pPacket->pSignalInfo->semaphore);
    replayResult = m_vkDeviceFuncs.SignalSemaphoreKHR(remappeddevice, pPacket->pSignalInfo);
    return replayResult;
}

VkResult vkReplay::manually_replay_vkWaitSemaphores(packet_vkWaitSemaphores* pPacket) {
    VkResult replayResult = VK_ERROR_VALIDATION_FAILED_EXT;
    VkDevice remappeddevice = m_objMapper.remap_devices(pPacket->device);
    if (pPacket->device != VK_NULL_HANDLE && remappeddevice == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in WaitSemaphores() due to invalid remapped VkDevice.");
        return VK_ERROR_VALIDATION_FAILED_EXT;
    }
    ...
    replayResult = m_vkDeviceFuncs.WaitSemaphores(remappeddevice, pPacket->pWaitInfo, pPacket->timeout);
    return replayResult;
}
    ...
    for (size_t i = amIdx + 1; !foundBindMem && i < portabilityTablePackets.size(); i++) {
        ...
        if (pPacketHeader1->packet_id == VKTRACE_TPI_VK_vkFreeMemory) {
            ...
            if (pFreeMemoryPacket->memory == traceAllocateMemoryRval) {
                ...
                vktrace_LogDebug("Memory allocated by vkAllocateMemory is not used.");
                break;
            }
        }
        ...
        if (pPacketHeader1->packet_id == VKTRACE_TPI_VK_vkBindImageMemory2KHR ||
            ...
            for (uint32_t bindCounter = 0; bindCounter < pBim2Packet->bindInfoCount; bindCounter++) {
                if (pBim2Packet->pBindInfos[bindCounter].memory == traceAllocateMemoryRval) {
                    ...
                    if (pPacketHeader1->packet_id == VKTRACE_TPI_VK_vkBindImageMemory2KHR || pPacketHeader1->packet_id == VKTRACE_TPI_VK_vkBindImageMemory2)
                        ...
                        if (traceBufferToASBuildSizes.find((VkBuffer)pBim2Packet->pBindInfos[bindCounter].image) != traceBufferToASBuildSizes.end())
            ...
            if (traceAllocateMemoryRval == bim->memory) {
                ...
                if (pPacketHeader1->packet_id == VKTRACE_TPI_VK_vkBindImageMemory)
                    ...
                    if (traceBufferToASBuildSizes.find((VkBuffer)bim->image) != traceBufferToASBuildSizes.end())
            ...
            if (((pPacketHeader1->packet_id == VKTRACE_TPI_VK_vkBindImageMemory || pPacketHeader1->packet_id == VKTRACE_TPI_VK_vkBindImageMemory2 || pPacketHeader1->packet_id == VKTRACE_TPI_VK_vkBindImageMemory2KHR) && pCreatePacketFull->packet_id == VKTRACE_TPI_VK_vkCreateImage) ||
                ...
                if (*(pCreatePacket->pImage) == bindMemImage) {
                    ...
                    if (pPacketHeader1->packet_id == VKTRACE_TPI_VK_vkBindImageMemory || pPacketHeader1->packet_id == VKTRACE_TPI_VK_vkBindImageMemory2 || pPacketHeader1->packet_id == VKTRACE_TPI_VK_vkBindImageMemory2KHR)
                        ...
                        if (traceBufferToASBuildSizes.find((VkBuffer)bindMemImage) != traceBufferToASBuildSizes.end())
...
VkResult vkReplay::manually_replay_vkAllocateMemory(packet_vkAllocateMemory *pPacket) {
    ...
    if (importAHWBuf) {
        ...
        if ((intptr_t)importAHWBuf->buffer >= (1 << 12)) {
            ...
            if (ret) {
                vktrace_LogError("Allocate android hardware buffer failed with return %d!, width = %u, height = %u, format = %p, usage = %p", ret, ahwbuf_desc->width, ahwbuf_desc->height, ahwbuf_desc->format, ahwbuf_desc->usage);
                return VK_ERROR_VALIDATION_FAILED_EXT;
            }
            ...
            ret = AHardwareBuffer_lockAndGetInfo(importAHWBuf->buffer, AHARDWAREBUFFER_USAGE_CPU_WRITE_OFTEN, -1, NULL, &ahwbuf_wrt_ptr, &outBytesPerPixel, &outBytesPerStride);
            if (ret || ahwbuf_wrt_ptr == nullptr) {
                vktrace_LogError("AM: AHardwareBuffer_lockAndGetInfo Lock the hardware buffer failed, use AHardwareBuffer_lock to lock the hardware buffer! ret = %d", ret);
                ret = AHardwareBuffer_lock(importAHWBuf->buffer, AHARDWAREBUFFER_USAGE_CPU_WRITE_OFTEN, -1, NULL, &ahwbuf_wrt_ptr);
                if (ret || ahwbuf_wrt_ptr == nullptr) {
                    vktrace_LogError("AM: AHardwareBuffer_lock lock the hardware buffer failed! ret = %d", ret);
                    return VK_ERROR_VALIDATION_FAILED_EXT;
                }
            } else {
                if (getAHardwareBufBPP(ahwbuf_desc->format) != outBytesPerPixel) {
                    vktrace_LogError("outBytesPerPixel = %d != getAHardwareBufBPP(ahwbuf_desc.format) = %d.", outBytesPerPixel, getAHardwareBufBPP(ahwbuf_desc->format));
                }
            }
            PBYTE psrc = importBufData + sizeof(AHardwareBuffer_Desc);
            PBYTE pdst = (unsigned char *)ahwbuf_wrt_ptr;
            vktrace_LogAlways("androidHardwareBufferPropertiesANDROID.allocationSize = %d, origSize = %d", androidHardwareBufferPropertiesANDROID.allocationSize, origSize);
            size_t additional_size = pPacket->header->size - ((uint64_t)psrc - (uint64_t)pPacket->header);
            if (androidHardwareBufferPropertiesANDROID.allocationSize == origSize && origSize <= additional_size) {
                memcpy(pdst, psrc, androidHardwareBufferPropertiesANDROID.allocationSize);
            } else {
            ...
            for (unsigned amIdx = amSearchPos; amIdx < portabilityTablePackets.size(); amIdx++) {
                ...
                if (pPacketHeader1->global_packet_index > pPacket->header->global_packet_index && pPacketHeader1->packet_id == VKTRACE_TPI_VK_vkBindBufferMemory) {
                    ...
                    if (*pPacket->pMemory == pBody->memory) {
                        if (traceBufferToASBuildSizes.find(pBody->buffer) != traceBufferToASBuildSizes.end()) {
    ...
    if (replayResult == VK_SUCCESS) {
        ...
        replayDeviceMemoryToSize[local_mem.replayDeviceMemory] = pPacket->pAllocateInfo->allocationSize;
#if defined(ANDROID)
        ...
        if (doAllocate) {
            ...
            vktrace_LogDebug("Skipping Allocate Memory 0x%llX, it is not bound to any image/buffer", *(pPacket->pMemory));
            return VK_SUCCESS;
        }
    }
    return replayResult;
...
void vkReplay::manually_replay_vkGetAccelerationStructureBuildSizesKHR(packet_vkGetAccelerationStructureBuildSizesKHR *pPacket) {
    ...
    VkAccelerationStructureKHR replaySrcAS = VK_NULL_HANDLE;
    VkAccelerationStructureKHR replayDstAS = VK_NULL_HANDLE;
    if (traceASToASCreateInfo.find(pPacket->pBuildInfo->srcAccelerationStructure) != traceASToASCreateInfo.end())
        replaySrcAS = m_objMapper.remap_accelerationstructurekhrs(pPacket->pBuildInfo->srcAccelerationStructure);
    if (traceASToASCreateInfo.find(pPacket->pBuildInfo->srcAccelerationStructure) != traceASToASCreateInfo.end())
        replayDstAS = m_objMapper.remap_accelerationstructurekhrs(pPacket->pBuildInfo->dstAccelerationStructure);
    ...
    for (uint32_t i = 0; i < pPacket->pBuildInfo->geometryCount; ++i) {
        if (pPacket->pBuildInfo->pGeometries[i].geometryType == VK_GEOMETRY_TYPE_TRIANGLES_KHR) {
            VkAccelerationStructureTrianglesOpacityMicromapEXT* pASTriangleOMM = (VkAccelerationStructureTrianglesOpacityMicromapEXT*)find_ext_struct((const vulkan_struct_header*)pPacket->pBuildInfo->pGeometries[i].geometry.triangles.pNext, VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_TRIANGLES_OPACITY_MICROMAP_EXT);
            if (pASTriangleOMM != nullptr) {
                VkMicromapEXT traceMicromap = pASTriangleOMM->micromap;
                pASTriangleOMM->micromap = m_objMapper.remap_micromapexts(traceMicromap);
                if (pASTriangleOMM->micromap == VK_NULL_HANDLE && traceMicromap != VK_NULL_HANDLE) {
                    vktrace_LogError("Error detected in GetAccelerationStructureBuildSizesKHR() due to invalid remapped micromap.");
                    return;
                }
            }
        }
    ...
    for (uint32_t i = 0; i < pPacket->infoCount; ++i) {
        for(uint32_t j = 0; j < pPacket->pInfos[i].geometryCount; ++j) {
            ...
            if (pGeometry->geometryType == VK_GEOMETRY_TYPE_TRIANGLES_KHR) {
                {
                    VkAccelerationStructureTrianglesOpacityMicromapEXT* pASTriangleOMM = (VkAccelerationStructureTrianglesOpacityMicromapEXT*)find_ext_struct((const vulkan_struct_header*)pGeometry->geometry.triangles.pNext, VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_TRIANGLES_OPACITY_MICROMAP_EXT);
                    if (pASTriangleOMM != nullptr) {
                        pASTriangleOMM->micromap = m_objMapper.remap_micromapexts(pASTriangleOMM->micromap);
                    }
                }
...
#if defined(_DEBUG)
        VkAccelerationStructureBuildSizesInfoKHR asBuildSizeInfo = {VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_BUILD_SIZES_INFO_KHR, NULL, 0, 0, 0};
...
void vkReplay::manually_replay_vkCmdBuildAccelerationStructuresKHR(packet_vkCmdBuildAccelerationStructuresKHR* pPacket) {
    ...
    for (uint32_t i = 0; i < pPacket->infoCount; i++) {
        ...
        for(uint32_t j = 0; j < pPacket->pInfos[i].geometryCount; j++) {
            if (pPacket->pInfos[i].pGeometries[j].geometryType == VK_GEOMETRY_TYPE_TRIANGLES_KHR) {
...
bool vkReplay::remapReplayPatternDeviceAddresssInMapedMemory(VkDevice remappedDevice, const void* pData, uint64_t size)
{
    ...
    if (g_pReplaySettings->specialPatternConfig == 1) {
        ...
        for (unsigned j = 0; j < size / sizeof(VkDeviceAddress); ++j) {
            ...
            if ((pDeviceAddress[j] < m_minTraceBufferDeviceAddress) || (pDeviceAddress[j] > (m_maxTraceBufferDeviceAddress + MAX_BUFFER_DEVICEADDRESS_SIZE))) {
                continue;
            }
...
VkResult vkReplay::manually_replay_vkFlushMappedMemoryRangesRemapAsInstanceSGHandle(packet_vkFlushMappedMemoryRangesRemapAsInstanceSGHandle *pPacket) {
    ...
    VkDevice remappedDevice = m_objMapper.remap_devices(pPacket->device);
    if (remappedDevice == VK_NULL_HANDLE) {
        vktrace_LogError("Skipping vkFlushMappedMemoryRangesRemapAsInstanceSGHandle() due to invalid remapped VkDevice.");
        return VK_ERROR_VALIDATION_FAILED_EXT;
    }
    ...
    for (uint32_t i = 0; i < pPacket->memoryRangeCount; i++) {
        ...
        if (m_objMapper.devicememory_exists(pPacket->pMemoryRanges[i].memory)) {
            localMem = &(m_objMapper.find_devicememory(pPacket->pMemoryRanges[i].memory));
                   ...
                   traceSkippedDeviceMemories.find(pPacket->pMemoryRanges[i].memory) != traceSkippedDeviceMemories.end()) {
            vktrace_LogDebug("Skipping vkFlushMappedMemoryRangesRemapAsInstanceSGHandle() due to VkDeviceMemory 0x%llX skipped in vkAllocateMemory.",
                             pPacket->pMemoryRanges[i].memory);
            return VK_SUCCESS;
        }
        ...
        localRanges[i].memory = m_objMapper.remap_devicememorys(pPacket->pMemoryRanges[i].memory);
        if (localRanges[i].memory == VK_NULL_HANDLE || localMem->pGpuMem == NULL) {
            vktrace_LogError("Skipping vkFlushMappedMemoryRangesRemapAsInstanceSGHandle() due to invalid remapped VkDeviceMemory.");
            return VK_ERROR_VALIDATION_FAILED_EXT;
        }
        ...
        for (int j = 1; j <= offsetCount; ++j) {
            ...
            if (it != traceDeviceAddrToReplayDeviceAddr4AS.end()) {
                vktrace_LogDebug("Package gid = %llu was generated by our vktrace_rq_pp. Remap acceleration structure address %llu to %llu", pPacket->header->global_packet_index, *addr, it->second.replayDeviceAddr);
        ...
        for (int j = 0; j < offsetCount; ++j) {
            ...
            vktrace_LogDebug("Package gid = %llu was generated by our vktrace_rq_pp. Remap shader group handle %d", pPacket->header->global_packet_index, sgHandleIdx);
...
VkResult vkReplay::manually_replay_vkUnmapMemoryRemapAsInstanceSGHandle(packet_vkUnmapMemoryRemapAsInstanceSGHandle* pPacket) {
    ...
    VkDevice remappedDevice = m_objMapper.remap_devices(pPacket->device);
    if (remappedDevice == VK_NULL_HANDLE) {
        vktrace_LogError("Skipping vkUnmapMemoryRemapAsInstanceSGHandle() due to invalid remapped VkDevice.");
        return VK_ERROR_VALIDATION_FAILED_EXT;
    }
    ...
    if (!m_objMapper.devicememory_exists(pPacket->memory)) {
        if (g_pReplaySettings->compatibilityMode && m_pFileHeader->portability_table_valid && !platformMatch() &&
            traceSkippedDeviceMemories.find(pPacket->memory) != traceSkippedDeviceMemories.end()) {
            vktrace_LogDebug("Skipping vkUnmapMemoryRemapAsInstanceSGHandle() due to VkDeviceMemory 0x%llX skipped in vkAllocateMemory.", pPacket->memory);
            return VK_SUCCESS;
        }
        vktrace_LogError("Skipping vkUnmapMemoryRemapAsInstanceSGHandle() due to invalid remapped VkDeviceMemory.");
        return VK_ERROR_VALIDATION_FAILED_EXT;
    }
...
VkResult vkReplay::manually_replay_vkFlushMappedMemoryRangesRemap(packet_vkFlushMappedMemoryRanges *pPacket) {
    ...
    VkDevice remappedDevice = m_objMapper.remap_devices(pPacket->device);
    if (remappedDevice == VK_NULL_HANDLE) {
        vktrace_LogError("Skipping vkFlushMappedMemoryRangesRemap() due to invalid remapped VkDevice.");
        return VK_ERROR_VALIDATION_FAILED_EXT;
    }
    ...
    for (uint32_t i = 0; i < pPacket->memoryRangeCount; i++) {
        ...
        if (m_objMapper.devicememory_exists(pPacket->pMemoryRanges[i].memory)) {
            localMem = &(m_objMapper.find_devicememory(pPacket->pMemoryRanges[i].memory));
                   ...
                   traceSkippedDeviceMemories.find(pPacket->pMemoryRanges[i].memory) != traceSkippedDeviceMemories.end()) {
            vktrace_LogDebug("Skipping vkFlushMappedMemoryRangesRemap() due to VkDeviceMemory 0x%llX skipped in vkAllocateMemory.",
                             pPacket->pMemoryRanges[i].memory);
            return VK_SUCCESS;
        }
        ...
        localRanges[i].memory = m_objMapper.remap_devicememorys(pPacket->pMemoryRanges[i].memory);
        if (localRanges[i].memory == VK_NULL_HANDLE || localMem->pGpuMem == NULL) {
            vktrace_LogError("Skipping vkFlushMappedMemoryRangesRemap() due to invalid remapped VkDeviceMemory.");
            return VK_ERROR_VALIDATION_FAILED_EXT;
        }
...
VkResult vkReplay::manually_replay_vkBindBufferMemory(packet_vkBindBufferMemory *pPacket) {
    ...
    if (g_pReplaySettings->compatibilityMode && m_pFileHeader->portability_table_valid && !platformMatch()) {
        ...
        memOffsetTemp = memOffsetTemp * replayGetBufferMemoryRequirements[remappedbuffer].alignment;
        pPacket->memoryOffset = memOffsetTemp;
    }

    if (replayDeviceMemoryToSize.find(remappedmemory) != replayDeviceMemoryToSize.end()
        && traceGetBufferMemoryRequirements.find(pPacket->buffer) != traceGetBufferMemoryRequirements.end()
        && replayGetBufferMemoryRequirements.find(remappedbuffer) != replayGetBufferMemoryRequirements.end()) {
        if (pPacket->memoryOffset + replayGetBufferMemoryRequirements[remappedbuffer].size > replayDeviceMemoryToSize[remappedmemory]) {
            vktrace_LogError("Error detected in BindBufferMemory() due to out of bounds[memoryOffset + mem_req.size > mem.allocationSize].");
            exit(1);
        }
    }
...
VkResult vkReplay::manually_replay_vkBindImageMemory(packet_vkBindImageMemory *pPacket) {
    ...
    if (replayDeviceMemoryToSize.find(remappedmemory) != replayDeviceMemoryToSize.end()
        && replayGetImageMemoryRequirements.find(remappedimage) != replayGetImageMemoryRequirements.end()) {
        if (memoryOffset + replayGetImageMemoryRequirements[remappedimage].size > replayDeviceMemoryToSize[remappedmemory]) {
            vktrace_LogError("Error detected in BindImageMemory() due to out of bounds[memoryOffset + mem_req.size > mem.allocationSize].");
            exit(1);
        }
    }
    if (pPacket->memory != VK_NULL_HANDLE && remappedmemory == VK_NULL_HANDLE) {
...
void vkReplay::manually_replay_vkGetImageMemoryRequirements2(packet_vkGetImageMemoryRequirements2 *pPacket) {
    ...
    if ((pPacket->device != VK_NULL_HANDLE) && (remappeddevice == VK_NULL_HANDLE)) {
        vktrace_LogError("Error detected in GetImageMemoryRequirements2() due to invalid remapped VkDevice.");
        return;
    }
...
void vkReplay::manually_replay_vkGetBufferMemoryRequirements2(packet_vkGetBufferMemoryRequirements2 *pPacket) {
    ...
    traceGetBufferMemoryRequirements[pPacket->pInfo->buffer] = pPacket->pMemoryRequirements->memoryRequirements;
...
void vkReplay::manually_replay_vkGetBufferMemoryRequirements2KHR(packet_vkGetBufferMemoryRequirements2KHR *pPacket) {
    VkDevice remappedDevice = m_objMapper.remap_devices(pPacket->device);
    if (remappedDevice == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in vkGetBufferMemoryRequirements2KHR() due to invalid remapped VkDevice.");
        return;
    }
...
VkResult vkReplay::manually_replay_vkCreateSampler(packet_vkCreateSampler *pPacket) {
    ...
    // According to Vulkan spec: VUID-VkSamplerCreateInfo-pNext-pNext
    if (pPacket->pCreateInfo->pNext && ((VkApplicationInfo*)(pPacket->pCreateInfo->pNext))->sType == VK_STRUCTURE_TYPE_SAMPLER_YCBCR_CONVERSION_INFO) {
        VkSamplerYcbcrConversionInfo *p = (VkSamplerYcbcrConversionInfo *)(pPacket->pCreateInfo->pNext);
...
void vkReplay::manually_replay_vkDestroySwapchainKHR(packet_vkDestroySwapchainKHR *pPacket) {
    ...
    for (auto it3 = replaySurfToSwapchain.begin(); it3 != replaySurfToSwapchain.end(); it3++) {
        if (it3->second == remappedswapchain) {
            replaySurfToSwapchain.erase(it3);
            break;
        }
    }
...
VkResult vkReplay::manually_replay_vkGetSwapchainImagesKHR(packet_vkGetSwapchainImagesKHR *pPacket) {
    ...
    if (pPacket->pSwapchainImages == nullptr && traceImageCount != *pPacket->pSwapchainImageCount) {
        ...
        vktrace_LogWarning("THe swapchain image count in trace file is different with replaying returned by vkGetSwapchainImageKHR, which may cause replay to display black.");
    }
    if (replayResult == VK_SUCCESS) {
        if (numImages != 0) {
...
VkResult vkReplay::manually_replay_vkQueuePresentKHR(packet_vkQueuePresentKHR *pPacket) {
    ...
    if (g_pReplaySettings->enableVirtualSwapchain && g_pReplaySettings->enableVscPerfMode == FALSE) {
        for (uint32_t i = 0; i < pPacket->pPresentInfo->swapchainCount; i++) {
            ...
            if (curSwapchainImage != VK_NULL_HANDLE && curSwapchainImage != traceImage) {
                traceImage = curSwapchainImage;
                curSwapchainImage = VK_NULL_HANDLE;
            }
            ...
            if (virtualImage != VK_NULL_HANDLE) {
                ...
                vktrace_LogError("Skipping vkQueuePresentKHR() due to getVirtualSwapchainImage return nullptr.");
                return VK_ERROR_VALIDATION_FAILED_EXT;
            }
        }
    }
...
VKAPI_ATTR VkBool32 VKAPI_CALL debugUtilsCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity, VkDebugUtilsMessageTypeFlagsEXT messageType,
    ...
    if (messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT) {
        vktrace_LogAlways("{%d} - {%s}: {%s}", callbackData->messageIdNumber, callbackData->pMessageIdName, callbackData->pMessage);
    }
...
VkResult vkReplay::manually_replay_vkDebugMarkerSetObjectNameEXT(packet_vkDebugMarkerSetObjectNameEXT *pPacket) {
    VkResult replayResult = VK_ERROR_VALIDATION_FAILED_EXT;
    VkDevice remappeddevice = m_objMapper.remap_devices(pPacket->device);
    if (pPacket->device != VK_NULL_HANDLE && remappeddevice == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in DebugMarkerSetObjectNameEXT() due to invalid remapped VkDevice.");
        return replayResult;
    }
    // No need to remap pNameInfo
    if (m_vkDeviceFuncs.DebugMarkerSetObjectNameEXT) {
        VkDebugMarkerObjectNameInfoEXT *pNameInfo = const_cast<VkDebugMarkerObjectNameInfoEXT*>(pPacket->pNameInfo);
        uint64_t remappedHandle = remapObjectHandleWithVkObjectType(pNameInfo->object, (VkObjectType)pNameInfo->objectType);
        if (remappedHandle == VK_OBJECT_TYPE_UNKNOWN) {
            vktrace_LogError("The object " PRIu64 " is currently unsupported.", pNameInfo->object);
            return VK_ERROR_VALIDATION_FAILED_EXT;
        }
        pNameInfo->object = remappedHandle;
        replayResult = m_vkDeviceFuncs.DebugMarkerSetObjectNameEXT(remappeddevice, pPacket->pNameInfo);
    }
    else {
        static bool DebugMarkerSetObjectNameEXT_warned = false;
        if (!DebugMarkerSetObjectNameEXT_warned) {
            vktrace_LogWarning("vkDebugMarkerSetObjectNameEXT doesn't supported on this device!\n");
            DebugMarkerSetObjectNameEXT_warned = true;
        }
        replayResult = VK_SUCCESS;
    }
    return replayResult;
...
VkResult vkReplay::manually_replay_vkDebugMarkerSetObjectTagEXT(packet_vkDebugMarkerSetObjectTagEXT *pPacket) {
    VkResult replayResult = VK_ERROR_VALIDATION_FAILED_EXT;
    VkDevice remappeddevice = m_objMapper.remap_devices(pPacket->device);
    if (pPacket->device != VK_NULL_HANDLE && remappeddevice == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in vkDebugMarkerSetObjectTagEXT() due to invalid remapped VkDevice.");
        return VK_ERROR_VALIDATION_FAILED_EXT;
    }
    VkDebugMarkerObjectTagInfoEXT *pTagInfo = const_cast<VkDebugMarkerObjectTagInfoEXT*>(pPacket->pTagInfo);
    if (pTagInfo != NULL) {
        uint64_t remappedHandle = remapObjectHandleWithVkObjectType(pTagInfo->object, (VkObjectType)pTagInfo->objectType);
        if (remappedHandle == VK_OBJECT_TYPE_UNKNOWN) {
            vktrace_LogError("The object " PRIu64 " is currently unsupported.", pTagInfo->object);
            return VK_ERROR_VALIDATION_FAILED_EXT;
        }
        pTagInfo->object = remappedHandle;
    }
    if (m_vkDeviceFuncs.DebugMarkerSetObjectTagEXT) {
        replayResult = m_vkDeviceFuncs.DebugMarkerSetObjectTagEXT(remappeddevice, pPacket->pTagInfo);
    } else {
        static bool DebugMarkerSetObjectTagEXT_warned = false;
        if (!DebugMarkerSetObjectTagEXT_warned) {
            vktrace_LogWarning("DebugMarkerSetObjectTagEXT doesn't supported on this device!\n");
            DebugMarkerSetObjectTagEXT_warned = true;
        }
        replayResult = VK_SUCCESS;
    }
    return replayResult;
...
void vkReplay::manually_replay_vkCmdPushDescriptorSetKHR(packet_vkCmdPushDescriptorSetKHR *pPacket) {
    ...
    if (pPacket->pDescriptorWrites != NULL) {
        for (uint32_t i = 0; i < pPacket->descriptorWriteCount && !errorBadRemap; i++) {
            pRemappedWrites[i].dstSet = m_objMapper.remap_descriptorsets(pRemappedWrites[i].dstSet);
            ...
            switch (pPacket->pDescriptorWrites[i].descriptorType) {
                    ...
                    for (uint32_t j = 0; j < pPacket->pDescriptorWrites[i].descriptorCount; j++) {
                        if (pPacket->pDescriptorWrites[i].pImageInfo[j].sampler != VK_NULL_HANDLE) {
                            const_cast<VkDescriptorImageInfo *>(pRemappedWrites[i].pImageInfo)[j].sampler =
                                m_objMapper.remap_samplers(pPacket->pDescriptorWrites[i].pImageInfo[j].sampler);
                            if (pRemappedWrites[i].pImageInfo[j].sampler == VK_NULL_HANDLE) {
                                vktrace_LogError("Skipping CmdPushDescriptorSetKHR() due to invalid remapped VkSampler.");
                                errorBadRemap = true;
                                break;
                            }
                        }
                    ...
                    for (uint32_t j = 0; j < pPacket->pDescriptorWrites[i].descriptorCount; j++) {
                        if (pPacket->pDescriptorWrites[i].pImageInfo[j].imageView != VK_NULL_HANDLE) {
                            const_cast<VkDescriptorImageInfo *>(pRemappedWrites[i].pImageInfo)[j].imageView =
                                m_objMapper.remap_imageviews(pPacket->pDescriptorWrites[i].pImageInfo[j].imageView);
                            if (pRemappedWrites[i].pImageInfo[j].imageView == VK_NULL_HANDLE) {
                                vktrace_LogError("Skipping CmdPushDescriptorSetKHR() due to invalid remapped VkImageView.");
                                errorBadRemap = true;
                                break;
                            }
                        }
                    ...
                    for (uint32_t j = 0; j < pPacket->pDescriptorWrites[i].descriptorCount; j++) {
                        if (pPacket->pDescriptorWrites[i].pTexelBufferView[j] != VK_NULL_HANDLE) {
                            const_cast<VkBufferView *>(pRemappedWrites[i].pTexelBufferView)[j] =
                                m_objMapper.remap_bufferviews(pPacket->pDescriptorWrites[i].pTexelBufferView[j]);
                            if (pRemappedWrites[i].pTexelBufferView[j] == VK_NULL_HANDLE) {
                                vktrace_LogError("Skipping CmdPushDescriptorSetKHR() due to invalid remapped VkBufferView.");
                                errorBadRemap = true;
                                break;
                            }
                        }
                ...
                case VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR:
                    {
                    VkWriteDescriptorSetAccelerationStructureKHR *pWriteDescriptorSetAccelerationStructureKHR = (VkWriteDescriptorSetAccelerationStructureKHR *)find_ext_struct((const vulkan_struct_header *)&(pPacket->pDescriptorWrites[i]), VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET_ACCELERATION_STRUCTURE_KHR);
                    for (int j = 0; pWriteDescriptorSetAccelerationStructureKHR != nullptr && j < pWriteDescriptorSetAccelerationStructureKHR->accelerationStructureCount; j++) {
                        const_cast<VkAccelerationStructureKHR*>(pWriteDescriptorSetAccelerationStructureKHR->pAccelerationStructures)[j] = m_objMapper.remap_accelerationstructurekhrs(pWriteDescriptorSetAccelerationStructureKHR->pAccelerationStructures[j]);
                    }
                    }
                    // Because AS handle can be 0x0, don't check the as.
                    break;
...
VkResult vkReplay::manually_replay_vkBindBufferMemory2KHR(packet_vkBindBufferMemory2KHR *pPacket) {
    ...
    for (size_t i = 0; i < pPacket->bindInfoCount; i++) {
        ...
        *((VkDeviceMemory *)&pPacket->pBindInfos[i].memory) = m_objMapper.remap_devicememorys(pPacket->pBindInfos[i].memory);
        ...
        if (replayDeviceMemoryToSize.find(pPacket->pBindInfos[i].memory) != replayDeviceMemoryToSize.end()
            && traceGetBufferMemoryRequirements.find(traceBuffer) != traceGetBufferMemoryRequirements.end()
            && replayGetBufferMemoryRequirements.find(remappedBuffer) != replayGetBufferMemoryRequirements.end()) {
            if (pPacket->pBindInfos[i].memoryOffset + replayGetBufferMemoryRequirements[remappedBuffer].size > replayDeviceMemoryToSize[pPacket->pBindInfos[i].memory]) {
                vktrace_LogError("Error detected in BindBufferMemory2KHR() due to out of bounds[memoryOffset + mem_req.size > mem.allocationSize].");
                exit(1);
            }
        }
    }
...
VkResult vkReplay::manually_replay_vkBindImageMemory2KHR(packet_vkBindImageMemory2KHR *pPacket) {
    ...
    for (size_t i = 0; i < pPacket->bindInfoCount; i++) {
        ...
        *((VkImage *)&pPacket->pBindInfos[i].image) = remappedImage;
        ...
        if (replayDeviceMemoryToSize.find(pPacket->pBindInfos[i].memory) != replayDeviceMemoryToSize.end()
            && replayGetImageMemoryRequirements.find(remappedImage) != replayGetImageMemoryRequirements.end()) {
            if (pPacket->pBindInfos[i].memoryOffset + replayGetImageMemoryRequirements[remappedImage].size > replayDeviceMemoryToSize[pPacket->pBindInfos[i].memory]) {
                vktrace_LogError("Error detected in BindImageMemory2KHR() due to out of bounds[memoryOffset + mem_req.size > mem.allocationSize].");
                exit(1);
            }
        }
    }
    ...
    for (size_t i = 0; i < pPacket->bindInfoCount; i++) {
        ...
        *(reinterpret_cast<VkDeviceMemory *>(&(const_cast<VkBindBufferMemoryInfo *>(pPacket->pBindInfos)[i]).memory)) =
        ...
        if (replayDeviceMemoryToSize.find(pPacket->pBindInfos[i].memory) != replayDeviceMemoryToSize.end()
            ...
            && replayGetBufferMemoryRequirements.find(remappedBuffer) != replayGetBufferMemoryRequirements.end()) {
            if (pPacket->pBindInfos[i].memoryOffset + replayGetBufferMemoryRequirements[remappedBuffer].size > replayDeviceMemoryToSize[pPacket->pBindInfos[i].memory]) {
                vktrace_LogError("Error detected in BindBufferMemory2() due to out of bounds[memoryOffset + mem_req.size > mem.allocationSize].");
                exit(1);
            }
        }
    }
    ...
    for (size_t i = 0; i < pPacket->bindInfoCount; i++) {
        ...
        *(reinterpret_cast<VkImage *>(&(const_cast<VkBindImageMemoryInfo *>(pPacket->pBindInfos)[i]).image)) = remappedImage;
        ...
        if (replayDeviceMemoryToSize.find(pPacket->pBindInfos[i].memory) != replayDeviceMemoryToSize.end()
            && replayGetImageMemoryRequirements.find(remappedImage) != replayGetImageMemoryRequirements.end()) {
            if (pPacket->pBindInfos[i].memoryOffset + replayGetImageMemoryRequirements[remappedImage].size > replayDeviceMemoryToSize[pPacket->pBindInfos[i].memory]) {
                vktrace_LogError("Error detected in BindImageMemory2() due to out of bounds[memoryOffset + mem_req.size > mem.allocationSize].");
                exit(1);
            }
        }
    }
...
void vkReplay::manually_replay_vkCmdPushConstantsRemap(packet_vkCmdPushConstants *pPacket) {
    ...
    VkCommandBuffer remappedcommandBuffer = m_objMapper.remap_commandbuffers(pPacket->commandBuffer);
    if (pPacket->commandBuffer != VK_NULL_HANDLE && remappedcommandBuffer == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CmdPushConstantsRemap() due to invalid remapped VkCommandBuffer.");
        return;
    }
    VkPipelineLayout remappedlayout = m_objMapper.remap_pipelinelayouts(pPacket->layout);
    if (pPacket->layout != VK_NULL_HANDLE && remappedlayout == VK_NULL_HANDLE) {
        vktrace_LogError("Error detected in CmdPushConstantsRemap() due to invalid remapped VkPipelineLayout.");
        return;
    }
    // No need to remap stageFlags
    // No need to remap offset
...
void vkReplay::on_terminate() {
    if (g_pReplaySettings->enablePipelineCache) {
        ...
        for (const auto &info: cache_list) {
            std::string &&full_path = m_pipelinecache_accessor->FindFile(UINT64_MAX, info.second, m_replay_gpu, m_replay_pipelinecache_uuid);
